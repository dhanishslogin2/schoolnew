...
class Dashboard_model extends CI_Model {
    ...
    public function get_summary_stats($academic_year_id = NULL)
    {
        ...
        $year_cond = $academic_year_id ? "AND s.academic_year_id = " . (int)$academic_year_id : '';
        ...
        $class_count = (int)$this->db->query("
            SELECT COUNT(*) AS cnt FROM tbl_classes WHERE is_deleted = 'n'
        ")->row()->cnt;
     ...
     * @return object
     */
    public function get_today_attendance($date = NULL)
    {
     ...
     * All four figures come from a single query.
     *
     * @return object
     */
    public function get_fees_summary()
    {
        $today        = date('Y-m-d');
        ...
        $row = $this->db->query("
            SELECT
                COALESCE(SUM(CASE WHEN payment_date = ? AND is_deleted='n' THEN amount_paid ELSE 0 END), 0)  AS today_collection,
                COALESCE(SUM(CASE WHEN payment_date >= ? AND is_deleted='n' THEN amount_paid ELSE 0 END), 0) AS monthly_collection
            FROM tbl_fee_payments
        ", [$today, $month_start])->row();

        // Pending = student_fees where status != 'paid' and is_deleted='n'
